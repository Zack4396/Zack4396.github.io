...
class configProjectV:
    def inbounds(self, allow_lan=True, protocol="http", port=10809):
        inbound_allow_lan = allow_lan
        inbound_protocol = protocol
        inbound_port = port

        assert isinstance(inbound_protocol, str), "inbound_protocol should be string"
        assert isinstance(inbound_port, int), "inbound_port should be integer"

        data = [
            {
                "tag": inbound_protocol,
                "port": inbound_port,
...
def main():
    ...
    with open(output_file, "w") as f:
        config = {
            "inbounds": configProjectV().inbounds(allow_lan, "http", http_port),
